...
export const Register = () => {
  ...
  const postCredentials = () => { // Saves the credentials of a new user to the database. Prints all to console first.
    ...
    fetch("https://localhost:5001/api/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(credentials) }).then(data => {
      console.log(data);
      //Get the user by email and password to then retrieve the id
      let url = 'https://localhost:5001/api/users/' + credentials.email + "/" + credentials.password;
      console.log(url);
