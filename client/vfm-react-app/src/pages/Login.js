...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    let url = 'http://localhost:5001/api/users/' + login.email + "/" + login.password;
    console.log(url);
    fetch(url).then(response => response.json()).then(data => {
      ...
      fetch('http://localhost:5001/curr-user-api/' + localStorage.getItem('curr_user_id')).then(response => response.json()).then(data => {
        console.log(data);
